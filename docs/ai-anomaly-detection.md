...
2. **Recommendation** (`anomaly-threshold-tuner.ts`) — `recommendThreshold` maps the measured rate to a conservative, one-step threshold change: above target → raise (stricter), well below → lower (recover sensitivity), inside the deadband or below `minSamples` → hold. Always clamped to **`[1.5, 8]`**. Note this band is *independent of* `ANOMALY_ZSCORE_THRESHOLD` (env range `≥ 0.5`, no max): if you set a base threshold outside `[1.5, 8]` **and** enable auto-tune, the first applied adjustment pulls it back into that band (audited).
