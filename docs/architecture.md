...
Artillery tree resolves `js-yaml@3.15.1` and `nanoid@3.3.18`, closing Dependabot alert IDs 238 and
241 without overrides because Artillery's declared ranges already admit both patches. A backend
security-regression test reads every matching lockfile entry and checks the published vulnerable
intervals per major release line; a global floor would incorrectly accept vulnerable later-major
versions such as `js-yaml@4.3.0`. This keeps the known ranges from returning even though CI
intentionally does not install the load-test tree.
