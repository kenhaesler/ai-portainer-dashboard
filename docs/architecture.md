...
**The gate.** `frontend/src/typecheck-gate.test.ts` follows the same "drive the shipped config, don't describe it" pattern as `backend/src/ci-audit-gate.test.ts` (#1578), `backend/src/packages-boundaries.test.ts` (#1585) and `backend/src/typecheck-gate.test.ts` (#1586). It parses the `typecheck` script for the `tsc` invocations it really performs, resolves each named config through TypeScript's own `getParsedCommandLineOfConfigFile` (so an exclusion arriving via `extends`, or an `include` that misses a directory, is caught), and asserts that every `*.test.ts(x)` file **discovered on disk** appears in the union of those programs. Discovery rather than a hardcoded list is the point: the file to catch is the next one added under a path some future `include` does not reach. It also asserts `vitest.setup.ts` is in the program (drop it and the 2241 matcher errors return, disguised as the tests being wrong), that no invocation emits, that the incremental invocations do not share a `tsBuildInfoFile`, and that the root script still chains `npm run typecheck -w frontend`. It deliberately does **not** assert that `tsconfig.json` keeps excluding tests — merging the two programs is a legitimate alternative design, so the gate checks coverage, not layout.
