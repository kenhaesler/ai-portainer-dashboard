-- Fix: UPDATE built-in profile prompts for existing DBs where INSERT OR IGNORE
-- in migration 026 was a no-op (rows already existed with empty prompts_json).
...
WHERE id = 'security-audit' AND is_built_in = 1;
...
SET prompts_json = '{
    ...
    updated_at = datetime('now')
WHERE id = 'devops' AND is_built_in = 1;
