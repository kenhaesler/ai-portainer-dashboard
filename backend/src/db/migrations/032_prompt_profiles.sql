-- Prompt Profiles: named collections of per-feature prompt configurations
-- Each profile stores all 11 feature prompts, model overrides, and temperature settings
-- Built-in profiles (is_built_in = 1) cannot be deleted but can be duplicated
...
INSERT OR IGNORE INTO prompt_profiles (id, name, description, is_built_in, prompts_json, created_at, updated_at)
VALUES (
  'default',
  'Default',
...
-- Set Default profile as the active profile
INSERT OR IGNORE INTO settings (key, value, category, updated_at)
VALUES ('prompts.active_profile', 'default', 'prompts', datetime('now'));
