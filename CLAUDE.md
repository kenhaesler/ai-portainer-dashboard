...
- **The load-test lockfile has advisory floors of `js-yaml@3.15.1` and `nanoid@3.3.18` (Dependabot alert IDs 238/241).** Both are Artillery transitive dependencies, and Artillery's existing `^3.14.1` / `^3.3.4` ranges accept the patched releases, so refresh the lockfile rather than adding overrides that would pin the tree forward. `backend/src/routes/security-regression-dependencies.test.ts` inspects every copy in `loadtests/package-lock.json` because CI does not install that tree. The vulnerable functions are not exposed by the dashboard runtime: `js-yaml` parses developer-supplied load-test configuration, while no repository load-test path calls Nano ID's custom generator with an externally supplied size. Still remove the vulnerable packages; reachability limits impact, not whether known-vulnerable code belongs in the toolchain.
