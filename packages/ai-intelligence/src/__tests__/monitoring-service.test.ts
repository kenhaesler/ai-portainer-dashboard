...
// DI pattern — security scanner injected via MonitoringDeps.scanner
const mockScanContainer = vi.fn().mockReturnValue([]);
...
// DI pattern — metrics injected via MonitoringDeps.metrics
const mockGetMovingAverage = vi.fn().mockReturnValue(null);
const mockGetMetrics = vi.fn().mockReturnValue([]);
const mockFindSimilarInsights = vi.fn().mockReturnValue([]);

// DI pattern — notifications injected via MonitoringDeps.notifications
const mockNotifyInsight = vi.fn().mockResolvedValue(undefined);

// DI pattern — operations injected via MonitoringDeps.operations
const mockSuggestAction = vi.fn().mockResolvedValue(null);
...
vi.mock('../services/adaptive-anomaly-detector.js', () => ({
...
vi.mock('../services/isolation-forest-detector.js', () => ({
  detectAnomalyIsolationForest: vi.fn().mockReturnValue(null),
}));

vi.mock('../services/log-analyzer.js', () => ({
...
// getCapacityForecasts mocked via DI (deps.metrics.getCapacityForecasts)
...
vi.mock('../services/monitoring-telemetry-store.js', () => ({
...
function makeDeps(): MonitoringDeps {
  return {
    scanner: {
      scanContainer: mockScanContainer,
    },
    metrics: {
      getLatestMetricsBatch: mockGetLatestMetricsBatch,
      getCapacityForecasts: mockGetCapacityForecasts,
      getMovingAverage: mockGetMovingAverage,
      getMetrics: mockGetMetrics,
      findSimilarInsights: mockFindSimilarInsights,
      // Unused in monitoring-service but required by MetricsInterface
    ...
    notifications: {
      notifyInsight: mockNotifyInsight,
    },
    operations: {
      suggestAction: mockSuggestAction,
    },
  };
}
...
describe('monitoring-service', () => {
  beforeEach(async () => {
    ...
    mockScanContainer.mockReturnValue([]);
    mockNotifyInsight.mockResolvedValue(undefined);
    mockSuggestAction.mockResolvedValue(null);
    ...
    // Create monitoring service with mocked deps
    const service = createMonitoringService(makeDeps());
    runMonitoringCycle = service.runMonitoringCycle;
  });
