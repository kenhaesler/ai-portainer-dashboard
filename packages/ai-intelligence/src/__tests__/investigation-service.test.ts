...
// DI pattern — metrics deps are injected via initInvestigationDeps, no @dashboard/observability needed
...
beforeAll(async () => {
  ...
  // Inject DI metrics deps once for all tests
...
describe('investigation-service', () => {
  beforeEach(async () => {
    await cache.clear();
    vi.restoreAllMocks();
    // Re-inject DI deps after restoreAllMocks clears vi.fn state
    initInvestigationDeps({
      getMetrics: mockGetMetrics,
      getMovingAverage: mockGetMovingAverage,
      generateForecast: mockGenerateForecast,
...
\`\`\``;
  ...
  describe('triggerInvestigation', () => {
    it('should skip when investigation is disabled', async () => {
      ...
      const mod = await import('../services/investigation-service.js');
      const insight = makeInsight();
