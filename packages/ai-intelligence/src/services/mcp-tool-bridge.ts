...
export async function routeToolCall(call: OllamaToolCall, infraLogs: InfrastructureLogsInterface): Promise<ToolCallResult> {
  ...
  const results = await executeToolCalls([{ tool: name, arguments: args }], infraLogs);
  return results[0] ?? { tool: name, success: false, error: 'Unknown tool' };
}

export async function routeToolCalls(calls: OllamaToolCall[], infraLogs: InfrastructureLogsInterface): Promise<ToolCallResult[]> {
  return Promise.all(calls.map((call) => routeToolCall(call, infraLogs)));
}
