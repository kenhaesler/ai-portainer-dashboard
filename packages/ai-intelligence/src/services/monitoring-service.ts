...
import type { SecurityFinding } from '@dashboard/contracts';

const log = createChildLogger('monitoring-service');

/** Local metric data shape — mirrors MetricInsert from @dashboard/observability */
...
export function createMonitoringService(deps: MonitoringDeps) {
  async function runMonitoringCycle(): Promise<void> {
    ...
    try {
      ...
      const endpoints = rawEndpoints.map((ep) => normalizeEndpointAsOf(ep, { referenceTimeMs }));
      ...
      const metricsFromDb: MetricData[] = [];
      ...
      try {
        batchMetrics = await deps.metrics.getLatestMetricsBatch(containerIds);
      } catch {
        log.warn(
      ...
      for (const container of allContainers) {
        const findings = deps.scanner.scanContainer(container.raw);
        const containerName =
          container.raw.Names?.[0]?.replace(/^\//, '') || container.raw.Id.slice(0, 12);
      ...
      for (const insight of allInsights) {
        ...
        if (insight.severity === 'critical' || insight.severity === 'warning') {
          deps.notifications.notifyInsight(insight as Insight).catch((err) =>
        ...
        const suggestedAction = await deps.operations.suggestAction(insight as Insight);
        if (suggestedAction) {
      ...
      if (insertedInsightsList.length > 0) {
        try {
          ...
          const correlation = await correlateInsights(
            storedInsights as Insight[],
            undefined,
            deps.metrics.findSimilarInsights,
          );
