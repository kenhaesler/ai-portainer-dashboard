...
import type { CapacityForecast } from '@dashboard/contracts';
...
type MovingAverageResult = { mean: number; std_dev: number; sample_count: number };
type MetricRow = { value: number; timestamp: string; [key: string]: unknown };

export interface InvestigationMetricsDeps {
  getMetrics(containerId: string, metricType: string, from: string, to: string): Promise<MetricRow[]>;
  getMovingAverage(containerId: string, metricType: string, windowSize: number): Promise<MovingAverageResult | null>;
  generateForecast(containerId: string, containerName: string, metricType: string): Promise<CapacityForecast | null>;
}

let metricsDeps: InvestigationMetricsDeps | null = null;

export function initInvestigationDeps(
  deps: InvestigationMetricsDeps,
): void {
  metricsDeps = deps;
}

function requireMetricsDeps(): InvestigationMetricsDeps {
  if (!metricsDeps) throw new Error('Investigation metrics deps not initialized - call initInvestigationDeps first');
  return metricsDeps;
}
...
async function gatherEvidence(insight: Insight): Promise<{
  ...
  // Gather metrics (only if deps initialized)
  if (insight.container_id && metricsDeps) {
    promises.push(
      (async () => {
        try {
          ...
          for (const metricType of ['cpu', 'memory']) {
            const avg = await deps.getMovingAverage(insight.container_id!, metricType, 30);
            if (avg) {
              const recent = await deps.getMetrics(
                insight.container_id!,
                metricType,
                from.toISOString(),
                now.toISOString(),
  ...
  // Gather capacity forecasts (only if deps initialized)
  if (insight.container_id && insight.container_name && metricsDeps) {
    try {
      const deps = requireMetricsDeps();
      const forecasts: CapacityForecast[] = [];
      for (const metricType of ['cpu', 'memory']) {
        const forecast = await deps.generateForecast(insight.container_id, insight.container_name, metricType);
