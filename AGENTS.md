...
This file provides guidance to AI coding assistants working with this repository. All AI tools (Copilot, Cursor, Windsurf, Cody, etc.) should follow these guidelines.
...
- **Observer-only**: This dashboard MUST NOT generate code that starts, stops, restarts, or otherwise mutates container state. Read-only access to Portainer only.
- **Never push directly to `main`**: All changes go through feature branches and pull requests.
- **Never commit secrets**: No `.env` files, API keys, passwords, or credentials in commits.
...
- **Test coverage required** — All new features and bug fixes must include corresponding unit and/or integration tests. Update existing tests when modifying behavior.
...
- JWT tokens must use strong secrets (32+ characters in production).
